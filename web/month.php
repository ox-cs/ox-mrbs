...
$room_param=get_form_var('room', 'string');
...
else
{
  ...
  if ($res)
  {
    ...
    echo "<li><a href=\"month.php?year=$year&amp;month=$month&amp;day=$day&amp;area=$area&amp;room=All\">";
    echo "<span>All rooms</span></a></li>\n";
    echo "</ul>\n";
  }
} // end select if
...
minicals($year, $month, $day, $area, $room_param, 'month');
echo "</div>\n";
...
$before_after_links_html = "<div class=\"screenonly\">
  <div class=\"date_nav\">
    <div class=\"date_before\">
      <a href=\"month.php?year=$yy&amp;month=$ym&amp;day=$yd&amp;area=$area&amp;room=$room_param\">
          &lt;&lt;&nbsp;".get_vocab("monthbefore")."
        </a>
    </div>
    <div class=\"date_now\">
      <a href=\"month.php?year=$cy&amp;month=$cm&amp;day=$cd&amp;area=$area&amp;room=$room_param\">
          ".get_vocab("gotothismonth")."
        </a>
    </div>
    <div class=\"date_after\">
       <a href=\"month.php?year=$ty&amp;month=$tm&amp;day=$td&amp;area=$area&amp;room=$room_param\">
...
// row[1] = End time
// row[2] = Entry ID
// This data will be retrieved day-by-day for the whole month
for ($day_num = 1; $day_num<=$days_in_month; $day_num++)
{
  if($room_param == "All")
  {
  $sql = "SELECT e.start_time, e.end_time, e.id, e.name, e.type,
          e.private, e.create_by, e.room_id, r.room_name
          FROM $tbl_entry AS e, $tbl_room AS r
          WHERE start_time <= $midnight_tonight[$day_num] AND end_time > $midnight[$day_num]
	  AND e.room_id=r.id
          ORDER by start_time";
  }
  else
      ...
      if (empty( $enable_periods ) )
      {
        switch (cmp3($row['start_time'], $midnight[$day_num]) . cmp3($row['end_time'], $midnight_tonight[$day_num] + 1))
        {
          ...
          case "= < ":         // Starts at midnight, ends before midnight
            $d[$day_num]["data"][] = $row['room_name']. ". " . htmlspecialchars(utf8_strftime(hour_min_format(), $row['start_time'])) . "~" . htmlspecialchars(utf8_strftime(hour_min_format(), $row['end_time']));
            break;
          case "> = ":         // Starts after midnight, ends at midnight
            $d[$day_num]["data"][] = $row['room_name']. ". " . htmlspecialchars(utf8_strftime(hour_min_format(), $row['start_time'])) . "~24:00";
            break;
          case "> > ":         // Starts after midnight, continues tomorrow
            $d[$day_num]["data"][] = $row['room_name']. ". " . htmlspecialchars(utf8_strftime(hour_min_format(), $row['start_time'])) . "~====&gt;";
            break;
          case "= = ":         // Starts at midnight, ends at midnight
            $d[$day_num]["data"][] = $row['room_name']. ". " . $all_day;
            break;
          case "= > ":         // Starts at midnight, continues tomorrow
            $d[$day_num]["data"][] = $row['room_name']. ". " . $all_day . "====&gt;";
            break;
          case "< < ":         // Starts before today, ends before midnight
            $d[$day_num]["data"][] = $row['room_name']. ". " . "&lt;====~" . htmlspecialchars(utf8_strftime(hour_min_format(), $row['end_time']));
            break;
...
echo "<thead>\n";
// ENC: Add the name of the room as a header

echo "<tr>\n";
echo "<th colspan=\"7\" style=\"border-bottom: 3px solid white; font-size: 150%\">" . $this_room_name . "</th>\n";
echo "</tr>\n";
...
for ($cday = 1; $cday <= $days_in_month; $cday++)
{
  ...
  {   
	...
	$booking_link = "view_entry.php?id=" . $d[$cday]["id"][$i] . "&amp;day=$cday&amp;month=$month&amp;year=$year&amp;room=" . $d[$cday]["room_id"][$i];
