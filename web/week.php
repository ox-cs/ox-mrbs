<?php
// $Id: week.php 1231 2009-10-27 16:52:17Z cimorrison $
...
// what term are we in?
  ...
  $date_difference=intval(($todays_date-$start_date)/(7*24*60*60));
...
// Show area and room:
echo "<h2 id=\"dwm\">";
echo "$this_area_name - $this_room_name";
echo " - $term_date week $date_difference";
echo "</h2>\n";
...
echo "<th colspan=\"8\" style=\"border-bottom: 3px solid white; font-size: 150%\">" . $this_room_name . "</th>\n";
echo "</tr>\n";
echo "<tr>\n";

  // the standard view, with days along the top and times down the side
  echo "<th class=\"first_last time\">".($enable_periods ? get_vocab("period") : get_vocab("time")).":</th>";
  for ($j = 0; $j<=($num_of_days-1) ; $j++)
  {
  ...
  for ($thisday = 0; $thisday<=($num_of_days-1) ; $thisday++, $row_class = ($row_class == "even_row")?"odd_row":"even_row")
  {
    ...
    else
    {
          ...
          )
      {
        ...
        $query_strings['booking']     = "area=$area&amp;room=$room&amp;day=$wday&amp;month=$wmonth&amp;year=$wyear";
  ...
  )
  {
    ...
    for ($thisday = 0; $thisday<=($num_of_days-1) ; $thisday++)
    {
      ...
      else
      {
        ...
        draw_cell($week_map[$room][$thisday][$time_t], $query_strings, $cell_class, $this_room_name);
      }
